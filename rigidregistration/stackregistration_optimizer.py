...
class imstack(object):
    ...
    def findImageShifts(self, correlationType="cc", findMaxima="pixel", verbose=True, clippingDimension=None):
        ...
        for i in range (0, self.nz-1):
            for j in range(i+1, self.nz):
                ...
                if findMaxima==self.getNShifts_gaussianFit:
                    xshift, yshift,nxshifts,nyshifts = findMaxima(cc)
        ...
        if findMaxima==self.getNShifts_gaussianFit:
            ...
            return self.X_ij, self.Y_ij, self.nX_ij, self.nY_ij
    ...
    def getSingleShift_gaussianFit(self,cc):
        ...
        # for intensity
        #shift_x, shift_y = positions[np.argmax(2*np.pi*amplitudes*sigmas[:,0]*sigmas[:,1]+offsets*np.pi**sigmas[:,0]*sigmas[:,1])]
        # for amplitude
        shift_x, shift_y=positions[np.argmax(offsets+amplitudes),:]
        return shift_x-np.shape(cc)[0]/2.0, shift_y-np.shape(cc)[1]/2.0

    #provides all functionality of getSingleShift_gaussianFit 
    #(i.e. calling this, should still be able to work with normal X_ij, Y_ij...)
    #but also 
    def getNShifts_gaussianFit(self,cc):
        ...
        return shift_x,shift_y,nshift_x,nshift_y
    ...
    def get_transitivity_scores(self,tX_ij,tY_ij,maxpaths=5):
        '''
        Calculates transitivity scores (self.transitivity_scores) for each element i,j of the shift matrix
        returns shift matrix transitivity score (matrix_transitivity), calculated as the sum of each elements transitivity score
        Transitivity score algorithm is from original get_outliers function

        tX_ij, tY_ij should just be shift matrices, the t is just to clearly differentiate 
        from the member variables...

        Note: currently don't bother flipping elements over the diagonal...
        '''
